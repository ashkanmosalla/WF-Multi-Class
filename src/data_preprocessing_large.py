...
def preprocess_large_data(file_path):
    ...
    # Drop rows where the 'LULC2' column has the value -9999
    df = df[df['LULC2'] != -9999]
    ...
    print(f"Columns in DataFrame after dropping: {df.columns.tolist()}")
    print(f"Number of columns in DataFrame: {len(df.columns)}")
    print(f"Number of new column names provided: {len(new_col_name)}")
...
if __name__ == "__main__":
    processed_df = preprocess_large_data('Export_RawData_MultiClass.csv')
